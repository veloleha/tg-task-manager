...
async def test_message_editing_without_thread_id():
    ...
    try:
        ...
        # Создаем фиктивный бот и вызываем edit_message_text напрямую
        mock_bot = MockBot()
        result = await mock_bot.edit_message_text(
        ...
        if result:
            print("   [OK] edit_message_text works correctly without message_thread_id")
            return True
        else:
            print("   [ERROR] edit_message_text failed")
            return False
...
async def test_error_handling():
    ...
    try:
        mock_bot = MockBotWithError()
        await mock_bot.edit_message_text(
            chat_id=-1001234567890,
            message_id=12345,
            text="Test message text"
        )
